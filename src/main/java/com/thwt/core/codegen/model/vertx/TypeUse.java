...
public abstract class TypeUse {
   ...
   */
  public static TypeUse createTypeUse(final AnnotatedType... reflectTypes) {
    return new TypeUse() {
...
//        TypeVariable<? extends Class<?>>[] typeVars = ((Class<?>) pt.getRawType()).getTypeParameters();
   ...
   * @return the created type use
   */
  public static TypeUse createTypeUse(final Types types, final TypeMirror... modelTypes) {
    return new TypeUse() {
      @Override
      public TypeUse getArg(int index) {
        // Only use the current types as the other types won't have the info anyway
        return createTypeUse(types, ((DeclaredType) modelTypes[0]).getTypeArguments().get(index));
      }
      ...
      private boolean isNullable(int index) {
        for (AnnotationMirror annotation : types.asElement(modelTypes[index]).getAnnotationMirrors()) {
