...
public abstract class Token {
   ...
   * @return the mapper
   */
  public static Function<Token, Token> tagMapper(final TypeElement ownerElt) {
    return new Function<Token, Token>() {
      ...
      public Token apply(Token token) {
        if (token.isInlineTag()) {
          ...
          if (tag.getName().equals("link")) {
            ...
            if (matcher.find()) {
              Element resolvedElt = Helper.resolveSignature(ownerElt,
                  matcher.group(1));
              if (resolvedElt != null) {
                ...
                if (resolvedTypeElt != null) {
                  ...
                  Tag.Link tagLink = new Tag.Link(
                      tag.getValue(),
                      resolvedElt,
                      TypeMirrorFactory.getInstance().create(resolvedType),
