...
//	private static final String[] VM_LIBRARY_FILES = { "/META-INF/template/_viewmodel_macros.vm","/META-INF/template/json_macros.vm",
//		"/META-INF/template/rest_macros.vm"
...
//		};
	...
	public void configure(Properties p, String[] vmLibraries) {
		logger.trace("Configuring Velocity engine...");
...
//			} else {
				velocityConfig.setProperty(RESOURCE_LOADER_CLASS,
						ClasspathResourceLoader.class.getCanonicalName());
//			}
			if(vmLibraries != null && vmLibraries.length > 0) {
			  velocityConfig.setProperty(VM_LIBRARY, StringUtils.join(vmLibraries, ","));
			}
...
//		}
	...
	public String render(String template, Map<String, Object> attributes) {
		...
		context.put("convert", new StringConvert());
